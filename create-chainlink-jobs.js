...
sh.sed('-i', /\"name\": \".*\"/, `"name": "OctoBay Claim ${timestamp}"`, './.jobs/twitter-post.json')
sh.sed('-i', /\"name\": \".*\"/, `"name": "OctoBay Claim ${timestamp}"`, './.jobs/twitter-followers.json')
...
sh.exec('chainlink jobs list', (code, output) => {
  ...
  sh.sed('-i', /^CHAINLINK_TWITTER_FOLLOWERS_JOB_ID=.*$/, 'CHAINLINK_TWITTER_FOLLOWERS_JOB_ID=' + jobIDs[4], 'contracts/.env')
  sh.sed('-i', /^CHAINLINK_TWITTER_POST_JOB_ID=.*$/, 'CHAINLINK_TWITTER_POST_JOB_ID=' + jobIDs[3], 'contracts/.env')
  sh.sed('-i', /^CHAINLINK_CLAIM_JOB_ID=.*$/, 'CHAINLINK_CLAIM_JOB_ID=' + jobIDs[0], 'contracts/.env')
  sh.sed('-i', /^CHAINLINK_RELEASE_JOB_ID=.*$/, 'CHAINLINK_RELEASE_JOB_ID=' + jobIDs[1], 'contracts/.env')
  sh.sed('-i', /^CHAINLINK_REGISTER_JOB_ID=.*$/, 'CHAINLINK_REGISTER_JOB_ID=' + jobIDs[2], 'contracts/.env')
})
